...
【思考过程】
- 需求分析：简述核心功能点
- 测试场景：列出需要覆盖的场景（必须考虑正常、边界、异常、大数据等）
- 优先级判断：说明P0/P1/P2/P3的分配依据

【测试用例】
格式模板：
用例ID|优先级|用例名称|前置条件|步骤1；步骤2；步骤3|预期1；预期2；预期3
示例1
TC001|Low|购物车下单成功|购物车有商品|1.进入购物车页->点击结算按钮；2.选择收货地址->提交订单；3.跳转支付页面->选择支付方式->确认支付|1.进入订单确认页；2.订单创建成功，显示订单号；3.支付成功，跳转订单详情页。
示例2（中间步骤无预期）：
TC002|High|分步表单填写|已登录|1.打开表单页->填写第一步信息->下一步；2.填写第二步信息->下一步；3.提交表单|1.无；2.无；3.提交成功，显示成功提示。

## 重要原则

- **完整性优先**：不要因为篇幅限制而减少测试用例数量。
- **禁止在任何用例中间截断输出；严格一行一条，无任何换行符。
- **禁止在步骤/预期里出现 `\n`、`\r` 等转义字符
- **严格按格式输出，不添加任何无关说明
...
### 复杂需求（5个以上功能点）
- 用例数量：10个以上
- 覆盖：正常场景5-10个，边界3-5个，异常3-5个，大数据3-5个

## 字段详细规则
### 用例名称
- 格式：简短一句话[模块/场景]下执行[操作]应[预期/校验点]，最好控制在15字内。
